...
export async function rescheduleSlot(req, res) {
  try {
    ...
    if (slot.interviewer.toString() !== currentUser._id.toString()) {
      return res
        .status(403)
        .json({ message: "Only the poster can edit this slot" });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .json({
          message: "duration must be a number between 15 and 240 minutes",
        });
    }
    ...
    await Promise.all(
      affected.map((userId) =>
        notify(
          ...
          slot._id,
          "rescheduled",
          `"${slot.title}" was rescheduled by the poster`,
        ),
      ),
    );
    ...
    res
      .status(500)
      .json({ message: "Failed to reschedule slot", error: err.message });
  }
}
...
export async function cancelSlot(req, res) {
  try {
    ...
    if (slot.interviewer.toString() !== currentUser._id.toString()) {
      return res
        .status(403)
        .json({ message: "Only the poster can cancel this slot" });
    }
    ...
    await Promise.all(
      affected.map((userId) =>
        notify(
          ...
          slot._id,
          "cancelled_by_poster",
          `"${slot.title}" was cancelled by the poster`,
        ),
      ),
    );
    ...
    res
      .status(500)
      .json({ message: "Failed to cancel slot", error: err.message });
  }
}
