...
export async function acceptRequest(req, res) {
  try {
    ...
    if (slot.interviewer.toString() !== currentUser._id.toString()) {
      return res.status(403).json({ message: "Only the poster can accept requests" });
    }
    ...
    const newBookedCount = slot.requests.filter((r) => r.status === "booked").length;
    if (newBookedCount >= slot.capacity) {
      slot.status = "full";
    }

    await slot.save();
    await notify(userId, slot._id, "booked", `You're booked for "${slot.title}"`);
  ...
  try {
    ...
    if (isPoster && !isSelf) {
      if (request.status !== "interested") {
        return res.status(400).json({ message: "Only interested requests can be declined" });
      }
      slot.requests = slot.requests.filter((r) => r.user.toString() !== userId);
      await slot.save();
      await notify(userId, slot._id, "declined", `The poster declined your request for "${slot.title}"`);
      return res.status(200).json({ message: "Request declined" });
    }

    if (isSelf) {
      if (request.status !== "booked") {
        return res.status(400).json({ message: "Only a booked request can be cancelled" });
      }
      slot.requests = slot.requests.filter((r) => r.user.toString() !== userId);
      ...
      const suggestion = nextInterested ? " Check the interested list to pick a replacement." : "";
      await notify(
        slot.interviewer,
        slot._id,
        "booking_cancelled",
    ...
    return res.status(403).json({ message: "Not authorized to remove this request" });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Failed to remove request", error: err.message });
  }
}